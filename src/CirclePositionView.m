...
- (void)drawRect:(CGRect)rect
{
    ...
    CGPoint imageCenter;
    CGFloat tempRad = self.radius/2 + ((self.radius/2)*[Game instance].numPlayers/C_MAX_NUM_PLAYERS);
    for(int i = 0; i < self.count; i++)
    {
        ...
        if(self.location == i || self.location == -1) 
            if(i < self.wolves)
                [self drawPositionForIcon:C_PLAYER_RED withMidPoint:imageCenter andSize:(tempRad/self.count)*4];
            else if(i == self.wolves && self.hunter)
                [self drawPositionForIcon:C_PLAYER_BROWN withMidPoint:imageCenter andSize:(tempRad/self.count)*4];
            else if((i == (self.wolves+1) && (self.healer && self.hunter)) || (i == (self.wolves) && (self.healer && !self.hunter)))
                [self drawPositionForIcon:C_PLAYER_BLUE withMidPoint:imageCenter andSize:(tempRad/self.count)*4];
            else
                [self drawPositionForIcon:C_PLAYER_GREEN withMidPoint:imageCenter andSize:(tempRad/self.count)*4];
        else
            [self drawPositionForIcon:C_DOT withMidPoint:imageCenter andSize:(tempRad/self.count)*4];
